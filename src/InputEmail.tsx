...
export const InputEmail = React.forwardRef((props, ref: React.Ref<HTMLInputElement>) => {
  const { name, label, defaultValue, placeholder = "you@example.com", hintText, error } = props as InputEmailType;
  ...
  return(
    <div className="max-w-sm">
      ...
      {hintText ? <p className="mt-2 text-sm text-gray-500" id="email-description">{hintText}</p> : undefined}      
      {error ? <p className="mt-2 text-sm text-red-600" id="email-error">{error}</p> : undefined}
    </div>    
  )
})